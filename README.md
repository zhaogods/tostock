...
Docker镜像：https://hub.docker.com/r/zsswwz/tostock 。
...
也可以直接编辑 `instock/lib/database.py` 中的默认值，或创建 `instock/config/database.json`（参考 `.env.example` 中的字段名）。优先级：环境变量 > .env > database.json > database.py 默认值。
...
2、配置 Token（推荐 .env 方式）
    编辑 `.env`，填入你的 Token：
```
TUSHARE_TOKEN=你的Tushare-Token
TUSHARE_DAILY_RATE=400
TUSHARE_DAILY_BASIC_RATE=150
TUSHARE_MONEYFLOW_RATE=150
TUSHARE_STOCK_BASIC_RATE=40
TUSHARE_RATE_LIMIT=180
```
    备选方式：编辑 `instock/config/tushare.json`：
```
{
...
```
3、积分要求
    2000积分即可覆盖个股行情(daily+daily_basic)和资金流(moneyflow)。
    升级到 6000积分可额外覆盖行业/概念板块资金流。
```
### 10.安装自动交易（可选）
...
docker run -dit --name InStock --network=InStockService \
    ...
    -e TUSHARE_TOKEN=你的Tushare-Token \
    -e TUSHARE_DAILY_RATE=400 \
    -e TUSHARE_DAILY_BASIC_RATE=150 \
    -e TUSHARE_MONEYFLOW_RATE=150 \
    -e TUSHARE_STOCK_BASIC_RATE=40 \
    -e TUSHARE_RATE_LIMIT=180 \
    zsswwz/tostock:latest
```
...
TUSHARE_DAILY_RATE        # daily 接口频率，历史行情也使用该频率
TUSHARE_DAILY_BASIC_RATE  # daily_basic 接口频率
TUSHARE_MONEYFLOW_RATE    # moneyflow 接口频率
TUSHARE_STOCK_BASIC_RATE  # stock_basic 接口频率
TUSHARE_RATE_LIMIT        # 批量历史缓存每分钟批次数
```
按自己数据库实际情况配置参数。
...
#回收容器
docker rmi zsswwz/tostock:latest library/mariadb:latest
#删除镜像
```
